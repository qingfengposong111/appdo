<template>
  <div>
    ...
    <mt-header title="奖金钱包明细" class="header">
    ...
    <div class="flex-box flex-justify-center top-title-flex">
      ...
      <div class="flex-1 font13 flex-justify-center">用户</div>
    ...
    <div class="main" v-if="dataList.length>0">
      ...
      <div class="flex-box flex-justify-center main-title-flex" v-for="(item,index) in dataList" :key="index">
        <div class="flex-1 font13 flex-justify-center">{{item.Time | formatDate}}</div>
        <div class="flex-1 font13 flex-justify-center">承租</div>
        <div class="flex-1 font13 flex-justify-center">{{item.recommendUsername}}</div>
        <div class="flex-1 font13 flex-justify-center">{{item.bonus}}</div>
      </div>
      </MeScroll>
    </div>
        ...
        Util.ajax.post('/api/recommend', {
          ...
          if (res.data.status === 200) {
            ...
            if (currIndex <= Number(res.data.data.pager.totalNum)) {
              ...
              that.dataList = that.dataList.concat(arr1);
              //隐藏下拉刷新的状态

<template>
  <div style="width: 100%;box-sizing: border-box">
    ...
    <mt-header title="修改银行帐户" class="header">
      <mt-button @click="goBack" slot="left" icon="back">返回</mt-button>
      <mt-button slot="right" class="single" @click="handleDelete">删除帐户</mt-button>
    </mt-header>
    <!-- 占位 -->
    <div style="width: 100%;height: 50px;"></div>
    <div class="head-zw"></div>
    <!-- main -->
    <div class="main-icon-flex" style="width: 100%;box-sizing: border-box">
      <div class="common-flex-view-this row flex-box-center">
        <div class="right-flex-view-common">持卡人</div>
        <div style="width: 80%;">
          <input style="width: 100%;" v-model.trim="userName" type="text">
        </div>
      </div>
      <!-- 银行名称 -->
      <div class="common-flex-view-this row flex-box-center">
    ...
    <div class="sexPicker" style="box-sizing: border-box">
      <mt-popup
        v-model="bankVisible"
        position="bottom"
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      handleDelete(){//删除帐户
        MessageBox.confirm("确认删除此帐户？", '删除提示').then(action => {
          Util.ajax.getByForm('/api/bank/delete', {
            params: {
      ...
      onValuesChange(picker, values) {//选择银行
        this.bankName = values[0];
      },
      getVal(){//监控输入的银行帐号
        if (this.bankName === '') {//是否选择了银行
      ...
      submit () {//确定提交
        if(this.flag){
          this.flag=false;
          if (!RegUtil.isMatchRealName(this.userName)) {//是否输入开户姓名
            TostUtil.tostTop('请输入开户姓名！');
            this.flag=true;
            return false;
          }
          if (!RegUtil.isMatchBankNumber(this.bankNumber)) {//是否输入银行卡号
            TostUtil.tostTop('请输入16~19位银行卡号！');
            this.flag=true;
            return false;
          }
          if (this.bankAddress === '') {//是否输入开户行地址
            TostUtil.tostTop('请输入开户行！');
            this.flag=true;
            return false;
          }
          Util.ajax.post('/api/bank/update', {
    ...
    created(){
      ...
      Util.ajax.getByForm('/api/bank/show', {
        params: {
          Id: this.$route.params.id
        }
      }).then(res => {
        if (res.data.status === 200) {
          this.bankName = res.data.data.bankName;//所属银行
          this.bankAddress = res.data.data.bankAddress;//开户行地址
          this.bankNumber = res.data.data.bankNumber;//银行卡号
          this.userName = res.data.data.bankAccountName;//开户人

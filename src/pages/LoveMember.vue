<template>
  <div>
    ...
    <mt-header title="激活会员" class="header">
    ...
    <div class="input-name-flex bg-tran-flex">
      1.激活会员需要消耗一个激活码<br>
      <span class="color-clone" @click="toGetCode">2.如何获取激活码？</span>
    </div>
    <!-- btn -->
  ...
  export default{
    ...
    created(){
      ...
      Util.ajax.getByForm('/api/member/balance').then(res => {
        ...
        if(res.data.status===200){
          this.num=res.data.data.code;//希望钱包
        }else{
    ...
    methods: {
      ...
      submit(){//提交
        ...
        Util.ajax.getByForm('/api/login/activation', {
          params: {

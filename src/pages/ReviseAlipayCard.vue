<template>
  <div>
    ...
    <mt-header title="修改支付宝帐户" class="header">
      <mt-button @click="goBack" slot="left" icon="back">返回</mt-button>
      <mt-button slot="right" class="single" @click="deleteAccount">删除帐户</mt-button>
    </mt-header>
    <!-- 占位 -->
...
<script>
  ...
  export default{
    data(){
      return {
        userName: JSON.parse(localStorage.getItem('loginInfo')).realName,//用户的真实姓名，用户登陆后获取或者填写
        flag:true,
        val: this.$route.params.name//支付宝帐户
      }
    },
    created(){
    },
    methods: {
      ...
      save () {//保存添加的支付宝帐户
        if(this.flag){
          ...
          Util.ajax.post('/api/alipay/update',{

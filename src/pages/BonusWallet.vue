<template>
  <div>
    ...
    <mt-header title="奖金钱包" class="header">
      <mt-button @click="goBack" slot="left" icon="back">返回</mt-button>
      <mt-button slot="right" class="single" @click="bonusRecord">明细</mt-button>
    </mt-header>
    ...
    <div class="btn-view-hope">
      <div class="btn-close-flex" @click="toBonus">退租AP</div>
    </div>
    <!--底部导航-->
    ...
    <div class="bot">
      ...
      <router-link to="/BonusWallet">
        <img src="static/images/icon/hope/money01.png" alt="">
        <div class="selected">奖金钱包</div>
      </router-link>
    ...
    created(){
      ...
      Util.ajax.getByForm('/api/member/balance').then(res => {
        if (res.data.status === 200) {
          this.APNumber = parseFloat(res.data.data.bonus).toFixed(4);//奖金钱包
